...
class COCODataSets(Dataset):
    ...
    def __load_data(self):
        ...
        for img_id in self.coco.imgs.keys():
            ...
            if self.remove_blank and valid_box_len == 0:
                ...
                continue
            box_info.shape = (width, height)
            box_info.padding_val = padding_val
    ...
    def collect_fn(batch: List[BoxInfo]):
        ...
        label_xyxy = np.concatenate(batch_target, axis=0)
        batch_id = np.array(sum([[i] * batch_length[i] for i in range(len(batch_length))], []))
        batch_target = torch.from_numpy(np.concatenate([batch_id[:, None], label_xyxy], axis=-1)).float()
    ...
    def set_transform(self):
        ...
        mosaic = MosaicWrapper(candidate_box_info=self.box_info_list,
                               ...
                               color_gitter=color_gitter, translate=self.aug_cfg['translate'])
