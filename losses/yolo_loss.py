...
class YOLOv5Builder(object):
    ...
    def __call__(self, predicts, targets, input_anchors):
        ...
        :param targets: [gt_num,6] (bs_id,label_idx,xc,yc,w,h){normalized w,h}
        ...
        target_cls, target_box, target_indices, target_anchors = list(), list(), list(), list()
        gain = torch.ones(7, device=device).float()
        # anchor_per_grid,gt_num
        anchor_idx = torch.arange(anchor_per_grid, device=device).float().view(anchor_per_grid, 1).repeat(1, num_gt)
        # [anchor_per_grid,num_gt,7](bs_idx,label_idx,xc,yc,w,h,anchor_idx)
        ...
        for i in range(num_layer):
            ...
            gain[2:6] = torch.tensor(predicts[i].shape)[[3, 2, 3, 2]]
            t = targets * gain
            if num_gt:
            ...
            b, c, a = t[:, [0, 1, 6]].long().T
            gt_xy = t[:, 2:4]
            gt_wh = t[:, 4:6]
            gij = (gt_xy - offsets).long()
            gi, gj = gij.T

            target_indices.append((b, a, gj.clamp_(0, gain[3]), gi.clamp_(0, gain[2])))
            target_box.append(torch.cat((gt_xy - gij, gt_wh), dim=1))
            target_anchors.append(anchors[a])
            target_cls.append(c)

        return target_cls, target_box, target_indices, target_anchors
...
class YOLOv5Loss(object):
    ...
    def __call__(self, predicts, targets, input_anchors):
        ...
        loss_cls, loss_box, loss_obj = torch.zeros(1, device=device), \
                                       ...
                                       torch.zeros(1, device=device)
        targets_cls, targets_box, targets_indices, targets_anchors = \
        ...
        loss_obj *= self.obj_weights * s * (1.4 if layer_num >= 4 else 1.)
        loss_cls *= cls_weights * s
        return loss_cls * num_bs, loss_box * num_bs, loss_obj * num_bs, match_num
